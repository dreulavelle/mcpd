...
something they can act on; `admin.writeProblem` substitutes the handler's own
sentence when the error names an internal package, and logs the real one. On
the client, `problemText` decides once whether the server's text is fit to
show: everything but a 500 is.

## Five from the change that wrote this
