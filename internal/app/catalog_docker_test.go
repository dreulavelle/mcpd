...
func TestCatalog_EverySourceOffLeavesNoCatalogue(t *testing.T) {
	...
	if catalog := buildCatalog(config.Catalog{}, nil, nil, log); catalog != nil {
	...
	only := buildCatalog(config.Catalog{Official: true}, nil, nil, log)
	...
	both := buildCatalog(config.Catalog{Official: true, Docker: true}, nil, nil, log)
