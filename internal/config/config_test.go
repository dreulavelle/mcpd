...
func TestEffectiveWarnings_AddressProblemsAreWarningsNotRefusals(t *testing.T) {
	for _, tc := range []struct {
		...
		{"a dashboard on https with no address for its certificate",
			Effective{PublicURL: "https://mcp.example.net", FrontendEnabled: true, FrontendTLSMode: "self-signed"},
			"covers only this machine"},
		{"a dashboard on https whose address says http",
			Effective{PublicURL: "https://mcp.example.net", FrontendEnabled: true, FrontendTLSMode: "custom",
				FrontendPublicURL: "http://mcpd.example.net"},
			"Change it to https"},
