...
func New(deps plugins.Deps, cfg Config) (*Plugin, error) {
	...
	return &Plugin{
		...
		client:     NewClient(http, cfg, token, user, pass, deps.Log, now, cache, observe),
	}, nil
}
...
//
// It makes the cheapest authenticated call there is -- one device, one page --
// which establishes that the address resolves, TLS works, the credential is
// accepted and the response is the API's JSON rather than a login page. Doing
// it at startup means a wrong token is a message on the dashboard rather than
// a confusing failure inside the first tool call an assistant makes.
func (p *Plugin) Start(ctx context.Context) error {
	if !p.configured {
		...
		p.deps.Log.Info("observium has no address or credentials yet; " +
	...
	if _, err := p.client.walk(ctx, "/devices", "devices", probe); err != nil {
		p.note(err)
		return err
	}
	p.note(nil)
	p.deps.Log.Info("observium ready", "address", redactURL(p.client.Root()))
	return nil
}
