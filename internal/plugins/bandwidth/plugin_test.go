...
func TestStartRefusesADefaultAccountTheCredentialDoesNotCover(t *testing.T) {
	...
	p := newFor(t, Config{
		...
		DefaultAccountID: "9000003", // real account, not one this credential covers
	...
	for _, want := range []string{"9000003", "9000001", "9000002"} {
...
func TestStartAcceptsACoveredAccount(t *testing.T) {
	_, srv := newTokenServer(t, 3600)
	p := newFor(t, Config{
		ClientID: "client", ClientSecret: "shh", DefaultAccountID: "9000002",
		APIURL: srv.URL, VoiceURL: srv.URL, MessagingURL: srv.URL,
	}, srv.Client())
...
func TestStartAcceptsACredentialThatNamesNoAccounts(t *testing.T) {
	...
	ts.accounts = "[]"
	p := newFor(t, Config{
		ClientID: "client", ClientSecret: "shh", DefaultAccountID: "9000001",
		APIURL: srv.URL, VoiceURL: srv.URL, MessagingURL: srv.URL,
	}, srv.Client())
...
func TestForbiddenNamesTheLikelyRole(t *testing.T) {
	for path, want := range map[string]string{
		"/api/v2/users/9000001/messages":                                   "Messaging insights",
		"/api/v2/accounts/9000001/statistics":                              "Reporting",
		"/v2/accounts/9000001/phoneNumberLookup/bulk/r-1":                  "TN lookup",
		"/api/v2/accounts/9000001/phoneNumbers/+1800/tollFreeVerification": "Campaign management",
	} {
...
func TestATruncatedListingSaysSo(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == tokenPath {
			...
			_, _ = io.WriteString(w, `{"access_token":"`+jwt(t, `{"accounts":["9000001"]}`)+
				`","expires_in":3600}`)
			return
		}
...
func TestACallMayNameAnyAccountTheCredentialCovers(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == tokenPath {
			_, _ = io.WriteString(w, `{"access_token":"`+
				jwt(t, `{"accounts":["9000001","9000002"]}`)+`","expires_in":3600}`)
			return
		}
	...
	for _, account := range []string{"9000001", "9000002"} {
		if _, err := p.listCalls(context.Background(), CallsInput{Account: account}); err != nil {
			t.Fatalf("listCalls on %s: %v", account, err)
		}
	}
	if len(asked) != 2 ||
		!strings.Contains(asked[0], "/accounts/9000001/") ||
		!strings.Contains(asked[1], "/accounts/9000002/") {
...
func TestACallNamingAnUncoveredAccountIsRefused(t *testing.T) {
	...
	_, err := p.listCalls(context.Background(), CallsInput{Account: "9000003"})
	if err == nil {
...
func TestTheDefaultAnswersAnUnqualifiedCall(t *testing.T) {
	...
	if len(asked) != 1 || !strings.Contains(asked[0], "/accounts/9000002/") {
