...
func TestGuardRefusesEverythingThatIsNotAllowedRead(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"listing calls": {
			http.MethodGet, "/api/v2/accounts/9000001/calls", true, "",
		},
		"reading one call": {
			http.MethodGet, "/api/v2/accounts/9000001/calls/c-123", true, "",
		},
		"the token exchange": {
			http.MethodPost, tokenPath, true, "",
		},
		"searching messages": {
			http.MethodGet, "/api/v2/users/9000001/messages", true, "",
		},
		"endpoints under the other prefix": {
			http.MethodGet, "/v2/accounts/9000001/endpoints", true, "",
		},
		// The write this credential is capable of and this plugin must never
		// make. A POST to /calls places a real telephone call.
		"placing a call": {
			http.MethodPost, "/api/v2/accounts/9000001/calls", false, "only reads",
		},
		"hanging up a call": {
			http.MethodDelete, "/api/v2/accounts/9000001/calls/c-123", false, "only reads",
		},
		"sending a message": {
			http.MethodPost, "/api/v2/users/9000001/messages", false, "only reads",
		},
		"an endpoint nobody added": {
			http.MethodGet, "/api/v2/accounts/9000001/campaigns", false, "allow-list",
		},
		...
		"downloading recording audio": {
			http.MethodGet, "/api/v2/accounts/9000001/calls/c-1/recordings/r-1/media",
			false, "allow-list",
		},

		// The Dashboard half. Reads allowed:
		"listing port-ins": {
			http.MethodGet, "/api/v2/accounts/9000001/portins", true, "",
		},
		"a port-in order's notes": {
			http.MethodGet, "/api/v2/accounts/9000001/portins/p-1/notes", true, "",
		},
		"E911 locations": {
			http.MethodGet, "/api/v2/accounts/9000001/e911s/locations", true, "",
		},
		"10DLC campaigns": {
			http.MethodGet, "/api/accounts/9000001/campaignManagement/10dlc/campaigns", true, "",
		},
		...
		"submitting a port-in": {
			http.MethodPost, "/api/v2/accounts/9000001/portins", false, "only reads",
		},
		"ordering numbers": {
			http.MethodPost, "/api/v2/accounts/9000001/orders", false, "only reads",
		},
		"disconnecting numbers": {
			http.MethodPost, "/api/v2/accounts/9000001/disconnects", false, "only reads",
		},
		"changing an E911 address": {
			http.MethodPut, "/api/v2/accounts/9000001/e911s/locations/l-1", false, "only reads",
		},
		"deleting a site": {
			http.MethodDelete, "/api/v2/accounts/9000001/sites/407", false, "only reads",
		},
		"creating a 10DLC campaign": {
			http.MethodPost, "/api/accounts/9000001/campaignManagement/10dlc/campaigns", false, "only reads",
		},
		...
		"downloading an LOA document": {
			http.MethodGet, "/api/v2/accounts/9000001/portins/p-1/loas/f-1", false, "allow-list",
		},
	} {
