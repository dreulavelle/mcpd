...
# itself a leak: one term per line in ~/.config/mcpd/sensitive-terms, or in the
# file MCPD_SENSITIVE_TERMS points at.
#
...
# hostname.
hosts=$(printf '%s\n' "$files" | xargs grep -H -n -E '(^|[^A-Za-z0-9_-])[a-z0-9-]+(\.[a-z0-9-]+)*\.(local|lan|intranet)([^A-Za-z0-9.-]|$)' 2>/dev/null \
    | grep -v -E '(^|[^A-Za-z0-9_-])(mcpd|nas|pbx|sso|host|server|router|switch|printer|plugin|example|acme|globex|cnmaestro|observium|graylog|bookstack)(\.[a-z0-9-]+)*\.(local|lan|intranet)' \
...
terms_file=${MCPD_SENSITIVE_TERMS:-$HOME/.config/mcpd/sensitive-terms}
if [ -f "$terms_file" ]; then
    tmp=$(mktemp)
    grep -v -E '^\s*(#|$)' "$terms_file" > "$tmp" || true
    if [ -s "$tmp" ]; then
        found=$(printf '%s\n' "$files" | xargs grep -H -n -i -F -f "$tmp" 2>/dev/null || true)
        report "a term from $terms_file:" "$found"
    fi
    rm -f "$tmp"
fi
